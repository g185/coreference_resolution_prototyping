...
class CorefModel(torch.nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.representation_start = FullyConnectedLayer(input_dim=768, hidden_size=1000, output_dim=768, dropout_prob=0.3)
        self.representation_end = FullyConnectedLayer(input_dim=768, hidden_size=1000, output_dim=768, dropout_prob=0.3)
        ...
        self.classification_head = torch.nn.Sequential(
            torch.nn.Linear(self.config.hidden_size, self.config.hidden_size // 2),
...
>>>>>>> ad95e0f8c154fae7b5977055ce475514db35b1e0
    ...
    def forward_as_BCE_classification(self, batch):
        ...
        for lhs, om, gold in zip(last_hidden_states, batch["offset_mapping"], batch["gold_edges"]):
            ...
            coref_logits = self.representation_start(lhs) @ self.representation_end(lhs).T
            ...
            loss.append(torch.nn.functional.binary_cross_entropy_with_logits(coref_logits, gold.flatten(), pos_weight=torch.tensor(10)))
    ...
    def forward_as_fra(self, batch):
        ...
        cartesian_matrix_idxs = torch.cartesian_prod(
            torch.arange(0, last_hidden_state.shape[1], device=last_hidden_state.device),
            torch.arange(0, last_hidden_state.shape[1], device=last_hidden_state.device)
        )

        representations = self.representation(last_hidden_state)

        x_pred, y_pred = cartesian_matrix_idxs[:, 0], cartesian_matrix_idxs[:, 1]

        outputs = self.cosine_similarity(representations.squeeze(0)[x_pred], representations.squeeze(0)[x_pred])

        gold = batch["gold_edges"][cartesian_matrix_idxs]
        output = {
            "loss": F.binary_cross_entropy_with_logits(outputs.values.flatten().unsqueeze(-1),
                                                       batch["gold_edges"].squeeze(0).flatten()
                                                       )
        }
